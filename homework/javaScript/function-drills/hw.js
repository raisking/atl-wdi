...
var sumOfNums = function(numsArray){
  // Your Code Here
  var sum = 0; //assigned a variable
  for (let i = 0; i < numsArray.length; i++){
    //loops until the 
    sum += numsArray[i]; // adds all loop nums
  }
  return sum;
};
...
var allGreaterThanTen = function(numsArray){
  // Your Code Here
  for(let i = 0; i < numsArray.length; i++){
    if (numsArray[i] > 10 || numsArray[i] === null){ // if num is > 10 or num is empty
      return true; //true
    }
    else{
      return false;
...
var noneHaveTwoOrMoreVowels = function(words){
  // Your Code Here
  for(var i = 0; i < words.length; i++){
    if(!hasAtLeastNVowels(words[i],0)){
      return false;
    }
  }
  return true;
